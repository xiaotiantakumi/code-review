...
def summarize_directory_contents(
    dir_path, include_gitignore, output_file="summary.txt"
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Summarize the contents of a directory"
    )
    parser.add_argument(
        "--path", required=True, help="Path to the directory to summarize"
    )
    parser.add_argument(
        "--include_ignore",
        type=bool,
        default=False,
        help="Include patterns from .gitignore",
    )
    parser.add_argument(
        "--output_file",
        default="summary.txt",
        help="File to write the summary to",
    )
    args = parser.parse_args()
    print(args)
    summarize_directory_contents(args.path, args.include_ignore, args.output_file)
